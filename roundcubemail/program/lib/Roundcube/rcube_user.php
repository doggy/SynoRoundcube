...
class rcube_user
{
	...
	// synology modify
	$cmd = '/usr/syno/bin/synofetch' . ' ' . escapeshellarg('http');
	@exec($cmd.' 6 ' , $Conf, $retval);
	$MailServerConf = array();
	
	if (0 == $retval && isset($Conf[0])) {
		$MailServerConf['account_type'] = $Conf[0];
		$MailServerConf['domain_name'] = $Conf[1];
	} else {
		$MailServerConf['account_type'] = 'local';
		$MailServerConf['domain_name'] = '';
	}
	
	$username = $user;
	if('ldap' == $MailServerConf["account_type"] ) {
		$username = $username . "@" . $MailServerConf["domain_name"];
	} else if ('win' == $MailServerConf["account_type"] ) {
		$username = $MailServerConf["domain_name"] . "\\" . $username;
	}
	...
	$sql_result = $dbh->query(sprintf($query, 'username'), $host, $username, $MailServerConf['account_type'], $MailServerConf['domain_name']);
 
        $sql_arr = $dbh->fetch_assoc($sql_result1);
	// end synology
